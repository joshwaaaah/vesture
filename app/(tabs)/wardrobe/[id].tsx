...
import { getWardrobeItemById } from '@/lib/wardrobe-items';
  ...
  if (!item) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center px-6">
        <AppText className="text-lg text-neutral-700">Item not found.</AppText>
      </SafeAreaView>
    );
  }
            ...
            <View className="border-t border-neutral-200 pt-4 mt-2 space-y-2">
              ...
              {item.color ? (
                <View className="flex-row justify-between">
                  ...
                  <AppText className="text-black text-lg">{item.color}</AppText>
                </View>
              ) : null}
              {item.size ? (
                <View className="flex-row justify-between">
                  <AppText className="text-neutral-500 text-lg">Size</AppText>
                  <AppText className="text-black text-lg">{item.size}</AppText>
                </View>
              ) : null}
            </View>
